package com.hyk.compress.sevenzip.Compression.LZMA;

import com.hyk.compress.sevenzip.Compression.RangeCoder.BitTreeEncoder;
import com.hyk.compress.sevenzip.Compression.LZMA.Base;
import com.hyk.compress.sevenzip.Compression.LZ.BinTree;
import com.hyk.compress.sevenzip.ICodeProgress;
...
public class Encoder
{
	...
	class LiteralEncoder
	{
		class Encoder2
		{
			...
			public void Init() { com.hyk.compress.sevenzip.Compression.RangeCoder.Encoder.InitBitModels(m_Encoders); }



			public void Encode(com.hyk.compress.sevenzip.Compression.RangeCoder.Encoder rangeEncoder, byte symbol) throws IOException
			{
				int context = 1;
				for (int i = 7; i >= 0; i--)
				{
			...
			public void EncodeMatched(com.hyk.compress.sevenzip.Compression.RangeCoder.Encoder rangeEncoder, byte matchByte, byte symbol) throws IOException
			{
				int context = 1;
				boolean same = true;
				for (int i = 7; i >= 0; i--)
			...
			public int GetPrice(boolean matchMode, byte matchByte, byte symbol)
			{
				...
				if (matchMode)
				{
					for (; i >= 0; i--)
					{
						int matchBit = (matchByte >> i) & 1;
						int bit = (symbol >> i) & 1;
						price += com.hyk.compress.sevenzip.Compression.RangeCoder.Encoder.GetPrice(m_Encoders[((1 + matchBit) << 8) + context], bit);
						context = (context << 1) | bit;
						if (matchBit != bit)
						{
							i--;
				...
				{
					int bit = (symbol >> i) & 1;
					price += com.hyk.compress.sevenzip.Compression.RangeCoder.Encoder.GetPrice(m_Encoders[context], bit);
					context = (context << 1) | bit;
				}
				return price;
			}
	...
	class LenEncoder
	{
		...
		{
			com.hyk.compress.sevenzip.Compression.RangeCoder.Encoder.InitBitModels(_choice);
		...
		public void Encode(com.hyk.compress.sevenzip.Compression.RangeCoder.Encoder rangeEncoder, int symbol, int posState) throws IOException
		{
		...
		{
			int a0 = com.hyk.compress.sevenzip.Compression.RangeCoder.Encoder.GetPrice0(_choice[0]);
			int a1 = com.hyk.compress.sevenzip.Compression.RangeCoder.Encoder.GetPrice1(_choice[0]);
			int b0 = a1 + com.hyk.compress.sevenzip.Compression.RangeCoder.Encoder.GetPrice0(_choice[1]);
			int b1 = a1 + com.hyk.compress.sevenzip.Compression.RangeCoder.Encoder.GetPrice1(_choice[1]);
			int i = 0;
	...
	com.hyk.compress.sevenzip.Compression.LZ.BinTree _matchFinder = null;
	com.hyk.compress.sevenzip.Compression.RangeCoder.Encoder _rangeEncoder = new com.hyk.compress.sevenzip.Compression.RangeCoder.Encoder();
	...
	void Create()
	{
		...
		{
			com.hyk.compress.sevenzip.Compression.LZ.BinTree bt = new com.hyk.compress.sevenzip.Compression.LZ.BinTree();
			int numHashBytes = 4;
	...
	void Init()
	{
		...
		com.hyk.compress.sevenzip.Compression.RangeCoder.Encoder.InitBitModels(_isMatch);
		com.hyk.compress.sevenzip.Compression.RangeCoder.Encoder.InitBitModels(_isRep0Long);
		com.hyk.compress.sevenzip.Compression.RangeCoder.Encoder.InitBitModels(_isRep);
		com.hyk.compress.sevenzip.Compression.RangeCoder.Encoder.InitBitModels(_isRepG0);
		com.hyk.compress.sevenzip.Compression.RangeCoder.Encoder.InitBitModels(_isRepG1);
		com.hyk.compress.sevenzip.Compression.RangeCoder.Encoder.InitBitModels(_isRepG2);
		com.hyk.compress.sevenzip.Compression.RangeCoder.Encoder.InitBitModels(_posEncoders);
	...
	{
		return com.hyk.compress.sevenzip.Compression.RangeCoder.Encoder.GetPrice0(_isRepG0[state]) +
				com.hyk.compress.sevenzip.Compression.RangeCoder.Encoder.GetPrice0(_isRep0Long[(state << Base.kNumPosStatesBitsMax) + posState]);
	}
	...
	int GetPureRepPrice(int repIndex, int state, int posState)
	{
		...
		if (repIndex == 0)
		{
			price = com.hyk.compress.sevenzip.Compression.RangeCoder.Encoder.GetPrice0(_isRepG0[state]);
			price += com.hyk.compress.sevenzip.Compression.RangeCoder.Encoder.GetPrice1(_isRep0Long[(state << Base.kNumPosStatesBitsMax) + posState]);
		}
		else
		{
			price = com.hyk.compress.sevenzip.Compression.RangeCoder.Encoder.GetPrice1(_isRepG0[state]);
			if (repIndex == 1)
				price += com.hyk.compress.sevenzip.Compression.RangeCoder.Encoder.GetPrice0(_isRepG1[state]);
			else
			{
				price += com.hyk.compress.sevenzip.Compression.RangeCoder.Encoder.GetPrice1(_isRepG1[state]);
				price += com.hyk.compress.sevenzip.Compression.RangeCoder.Encoder.GetPrice(_isRepG2[state], repIndex - 2);
			}
		}
		return price;
	}
	...
	int GetOptimum(int position) throws IOException
	{
		...
		_optimum[1].Price = com.hyk.compress.sevenzip.Compression.RangeCoder.Encoder.GetPrice0(_isMatch[(_state << Base.kNumPosStatesBitsMax) + posState]) +
				_literalEncoder.GetSubCoder(position, _previousByte).GetPrice(!Base.StateIsCharState(_state), matchByte, currentByte);
		_optimum[1].MakeAsChar();

		int matchPrice = com.hyk.compress.sevenzip.Compression.RangeCoder.Encoder.GetPrice1(_isMatch[(_state << Base.kNumPosStatesBitsMax) + posState]);
		int repMatchPrice = matchPrice + com.hyk.compress.sevenzip.Compression.RangeCoder.Encoder.GetPrice1(_isRep[_state]);
		...
		int normalMatchPrice = matchPrice + com.hyk.compress.sevenzip.Compression.RangeCoder.Encoder.GetPrice0(_isRep[_state]);
		...
		while (true)
		{
			...
			int curAnd1Price = curPrice +
				com.hyk.compress.sevenzip.Compression.RangeCoder.Encoder.GetPrice0(_isMatch[(state << Base.kNumPosStatesBitsMax) + posState]) +
			...
			matchPrice = curPrice + com.hyk.compress.sevenzip.Compression.RangeCoder.Encoder.GetPrice1(_isMatch[(state << Base.kNumPosStatesBitsMax) + posState]);
			repMatchPrice = matchPrice + com.hyk.compress.sevenzip.Compression.RangeCoder.Encoder.GetPrice1(_isRep[state]);
			...
			if (!nextIsChar && matchByte != currentByte)
			{
				...
				if (lenTest2 >= 2)
				{
					...
					int nextRepMatchPrice = curAnd1Price +
						com.hyk.compress.sevenzip.Compression.RangeCoder.Encoder.GetPrice1(_isMatch[(state2 << Base.kNumPosStatesBitsMax) + posStateNext]) +
						com.hyk.compress.sevenzip.Compression.RangeCoder.Encoder.GetPrice1(_isRep[state2]);
					{
				...
				if (lenTest < numAvailableBytesFull)
				{
					...
					if (lenTest2 >= 2)
					{
						...
						int curAndLenCharPrice =
								...
								com.hyk.compress.sevenzip.Compression.RangeCoder.Encoder.GetPrice0(_isMatch[(state2 << Base.kNumPosStatesBitsMax) + posStateNext]) +
						...
						int nextMatchPrice = curAndLenCharPrice + com.hyk.compress.sevenzip.Compression.RangeCoder.Encoder.GetPrice1(_isMatch[(state2 << Base.kNumPosStatesBitsMax) + posStateNext]);
						int nextRepMatchPrice = nextMatchPrice + com.hyk.compress.sevenzip.Compression.RangeCoder.Encoder.GetPrice1(_isRep[state2]);
			...
			if (newLen >= startLen)
			{
				normalMatchPrice = matchPrice + com.hyk.compress.sevenzip.Compression.RangeCoder.Encoder.GetPrice0(_isRep[state]);
	...
	void FillDistancesPrices()
	{
		...
		for (int lenToPosState = 0; lenToPosState < Base.kNumLenToPosStates; lenToPosState++)
		{
			...
			for (posSlot = Base.kEndPosModelIndex; posSlot < _distTableSize; posSlot++)
				_posSlotPrices[st + posSlot] += ((((posSlot >> 1) - 1) - Base.kNumAlignBits) << com.hyk.compress.sevenzip.Compression.RangeCoder.Encoder.kNumBitPriceShiftBits);
